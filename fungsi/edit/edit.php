...
if (isset($_GET['user']) && $_GET['user'] == 'edit') {
  ...
  if (!empty($_POST['password'])) {
    ...
    $sql = "UPDATE login SET user=?, pass=?, id_member=?, jabatan=? WHERE id_login=?";
    $row = $config->prepare($sql);
    $row->execute([$username, $password, $id_member, $jabatan, $id]);
  } else {
    $sql = "UPDATE login SET user=?, id_member=?, jabatan=? WHERE id_login=?";
    $row = $config->prepare($sql);
    $row->execute([$username, $id_member, $jabatan, $id]);
  }

  header("Location: ../../index.php?page=tambah&success=edit");
}


if (isset($_GET['supplier']) && $_GET['supplier'] === 'edit') {
  $id     = $_POST['id'];
  $nama   = $_POST['nama'];
  $no_wa  = $_POST['no_wa'];
  $alamat = $_POST['alamat'];

  $stmt = $config->prepare("UPDATE supplier SET nama = ?, no_wa = ?, alamat = ? WHERE id = ?");
  if ($stmt->execute([$nama, $no_wa, $alamat, $id])) {
    header("Location: ../../index.php?page=supplier&success=edit");
    exit;
  } else {
